...
const yearsInService = new Date().getFullYear() - foundingYear;
const officeAddress = ["4001 McEwen Rd Suite 404", "Dallas, TX 75244"];
const mailingAddress = ["6841 Virginia Pkwy", "Suite 103-402", "McKinney, TX 75071"];
...
const features = [
  ...
  ["box", "Authority you can verify", "Active interstate operating authority with a satisfactory safety rating."],
];
...
const credentials = [
  ["badge", "Operating Authority", "Active interstate authority for property transportation."],
  ...
  ["clock", "Track Record", {
    type: "years-in-service",
    prefix: `In business since ${foundingYear} - over `,
    suffix: " years moving freight.",
  }],
];

const dieselTable = {
  releaseDate: "September 1, 2026",
  nextReleaseDate: "September 9, 2026",
  columns: ["Region", "08/17/26", "08/24/26", "08/31/26", "Week Ago", "Year Ago"],
  rows: [
...
const articles = [
  ...
  },
  {
    ...
    sections: [
      ...
      ["The Daats standard", [
        "Daats Companies was built around a straightforward promise: Service with Integrity. For shippers, that means working with a carrier whose authority can be checked, whose safety posture is part of the conversation, and whose team understands that freight decisions carry real consequences. The company's active interstate authority is part of the public record shippers can use to verify who they are trusting.",
...
function footer(depth = 0) {
  return `<footer class="site-footer" id="contact"><div class="container footer-grid"><div class="footer-brand">${brand(depth)}<p>Full-service trucking out of Dallas, Texas. Dry-van and refrigerated freight, delivered with Service with Integrity.</p></div><nav class="footer-col" aria-label="Services"><h3>Services</h3><ul>${services.map(([, title]) => `<li><a href="${pathTo("services.html", depth)}#${slug(title)}">${title}</a></li>`).join("")}</ul></nav><nav class="footer-col" aria-label="Company"><h3>Company</h3><ul><li><a href="${pathTo("about.html", depth)}">About</a></li><li><a href="${pathTo("services.html", depth)}">Services</a></li><li><a href="${pathTo("industries.html", depth)}">Industries</a></li><li><a href="${pathTo("coverage.html", depth)}">Coverage</a></li><li><a href="${pathTo("safety.html", depth)}">Safety</a></li><li><a href="${careersUrl}" target="_blank" rel="noopener noreferrer">Apply</a></li><li><a href="${pathTo("blog.html", depth)}">Blog</a></li><li><a href="${careersUrl}" target="_blank" rel="noopener noreferrer">Driver Application</a></li></ul></nav><div class="footer-col"><h3>Contact</h3><ul><li class="contact-row">${icon("pin")}<span>Office:<br>${officeAddress.join("<br>")}</span></li><li class="contact-row">${icon("pin")}<span>Mailing:<br>${mailingAddress.join("<br>")}</span></li><li class="contact-row">${icon("phone")}<span><a href="tel:+19725604040">972-560-4040</a> (main)<br><a href="tel:+19725604041">972-560-4041</a> (dispatch)</span></li><li class="contact-row">${icon("printer")}<span>972-560-4045 (fax)</span></li><li class="contact-row">${icon("mail")}<span><a href="mailto:info@daatscompanies.com">info@daatscompanies.com</a><br><a href="mailto:dispatch@daatscompanies.com">dispatch@daatscompanies.com</a></span></li></ul></div></div><div class="container footer-bottom"><p>&copy; 2026 Daats Companies, Inc. All rights reserved.</p><p class="footer-tagline">SERVICE WITH INTEGRITY</p></div></footer>`;
}
...
function renderCardText(text) {
  ...
  if (text?.type === "years-in-service") {
    return `${esc(text.prefix)}<span data-years-in-service>${String(yearsInService)}</span>${esc(text.suffix)}`;
  }
  throw new TypeError("Unsupported card text");
}

function cards(items, extra = "") {
  return `<div class="card-grid ${extra}">${items.map(([ic, title, text]) => `<article class="service-card" id="${slug(title)}"><span class="card-icon">${icon(ic)}</span><h3>${esc(title)}</h3><p>${renderCardText(text)}</p><a class="card-link" href="#contact">Learn more -></a></article>`).join("")}</div>`;
}
